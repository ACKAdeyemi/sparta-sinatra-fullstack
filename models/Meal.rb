class Meal
  ...
  def self.find id
    ...
    sql = "SELECT * FROM meals WHERE id=#{ id } LIMIT 1"

    # Get the raw results
    meals = conn.exec(sql)

    # Result comes back as array so need to run hydrate on first instance
    meal = self.hydrate meals[0]

    # Return the cleaned meal
    meal
  end

  def save
    conn = Meal.open_connection

    # If the object that the save method is run on does NOT have an existing ID, create a new instance
    if (!self.id)
      sql = "INSERT INTO meals (title, description) VALUES ('#{ self.title }','#{ self.description }')"
    else
      sql = "UPDATE meals SET title='#{self.title}', description='#{self.description}' WHERE id='#{self.id}'"
    end
